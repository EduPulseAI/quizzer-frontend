    ...
    <div className="space-y-4">
      {currentQuestion.choices.map((option, index) => {
        ...
        if (isSelected && option.isCorrect) {
          ...
          buttonClass += 'border-green-500 bg-green-500/20 text-green-300 shadow-[0_0_15px_rgba(34,197,94,0.3)]';
        } else if (isSelected && !option.isCorrect) {
          // Selected wrong answer - red with thick border (but can try again)
          buttonClass += 'border-red-500 bg-red-500/20 text-red-300 shadow-[0_0_15px_rgba(239,68,68,0.3)]';
        } else {
            ...
            <div className="flex items-center gap-4">
              ...
              {isSubmitting && isSelected && (
                <span className="ml-auto">
                  <span className="animate-pulse text-blue-400 text-sm">Submitting...</span>
                </span>
              )}
              {!isSubmitting && hasAnswered && isSelected && option.isCorrect && (
